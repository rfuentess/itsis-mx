...
---
-- Will generate a random DUID with a valid format.
-- This is going to be: link-layer address plus time [DUID-LLT] (RFC 3315 9.2 p. 20)
...
function Generar_DUID ()
  ...
  if stime > 946684800 then
    ...
    stime = stime - 946684800 -- For now, we only subtract 30 years (more or less)
  end
...
local Spoof_Host_Solicit = function ()
  ...
  -- The Option Request field it-s optional, however seem to be that all the
  -- clients use it.
  -- Yet some servers (Wide-Server, Windows Server 2008/2012 server) Don't need it.
  if Bool_Option_Req ~= nil then
    Option_Request = Generar_Option_Request()
  end

  stdnse.print_verbose(3, SCRIPT_NAME .. ".Solicit: " .. " New SOLICIT Message. ID: "
                        .. TransactionID)
  stdnse.print_verbose(3, SCRIPT_NAME .. ".Solicit: " .. " Client ID: " .. ClientID)
...
-- @param   String  A string representing HEXADECIMAL data (The SOLICIT message)
...
local Transmision_Recepcion = function (IPv6src, IPv6dst, Prtsrc, Prtdst, Mensaje,
                                        Interface, Subnet)
  ...
  -- for retrieve our own packets (or risk to lost our message because we turn on
  -- too late).
  --    device: The dnet-style interface name of the device you want to capture from.
  --    snaplen: The length of each packet you want to capture (similar to the
  --    -s option to tcpdump).
  --    promisc: Boolean value for whether the interface should activate promiscuous
  --    mode.
  ...
  if type(Subnet) == "table" then
    ...
    if Binario ~= nil then
      ...
      stdnse.print_verbose(3, SCRIPT_NAME .. 
                   "\t\t The next provided subnet has wrong syntax: " .. Subnet ..
                   mensaje)
    end
...
function prerule ()
  ...
  elseif iface.link ~= "ethernet" then
    stdnse.print_verbose(1, " The NSE Script need to work with a Ethernet" ..
                   "Interface, Please use the argument -e <Interface> to select it.")
    return false
  end
...
function action ()
  ...
  for Index, Subnet in ipairs(UserSubnets) do
    ...
    bRecorrido = Transmision_Recepcion(Spoofed_IPv6Address, "FF02::1:2", 546, 547,
                                       Relay, nmap.get_interface(), Subnet)
